...
def main():
    ...
    ecs_cluster, sg_alb, sg_cluster, launch_config = create_ecs_cluster(cfg['cluster_name'], env['instance_type'], env['ami_id'] , env['min_instance'], env['max_instance'])
#    ts.add(create_ecs_cluster(cfg['cluster_name'], env['instance_type'], env['ami_id'] , env['min_instance'], env['max_instance']))

    print(dump())


def create_ecs_cluster(cluster_name, instance_type, ami_id, min_size, max_size):
    """ECS Cluster Module"""
